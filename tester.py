...
class BacktestConfig:
    ...
    max_alloc_pct: float = 0.12        # הקצאה מקסימלית למניה אחת
    max_positions: int = 10            # <<< מקסימום 10 פוזיציות פתוחות במקביל כפי שביקשת
    ...
    # סינון עוצמה יחסית - רק מניות שחזקות מהשוק!
    min_rs_65: float = 0.02            # <<< המניה חייבת להכות את המדד ב-2% לפחות ב-65 הימים האחרונים
    
    # ניהול טרייד (סטופ הדוק וסבלנות למנצחים)
    ...
    max_hold_bars: int = 250           # <<< מאפשרים למניה לרוץ עד שנה שלמה אם היא בטרנד!
    
    use_point_in_time_universe: bool = False
    raw_price_mode: bool = False
    allow_same_day_cash_reuse: bool = False
    universe_file: str | None = None
    output_prefix: str = "ascending_triangle_v11"
...
# ==========================================
# 5. Patient Trade Simulation (TIGHT STOP & LET WINNERS RUN)
# ==========================================
def simulate_trade(df: pd.DataFrame, entry_date: pd.Timestamp, entry_price: float, tight_low: float, cfg: BacktestConfig):
    ...
    for i, row in enumerate(future.itertuples()):
        ...
        # --- מנגנון "תנו למנצחים לרוץ" משופר ---
        if profit >= 0.06: stop_price = max(stop_price, entry_price * 1.005) # ברייק איוון
        if profit >= 0.15: stop_price = max(stop_price, highest_seen * 0.90) # נגרר הדוק בהתחלה
        if profit >= 0.30: stop_price = max(stop_price, highest_seen * 0.85) # ברגע שזה טרנד גדול, נותנים למניה 15% לנשום
...
def generate_candidates(tickers, data_cache, spy_df, cfg):
    ...
    for ticker in tqdm(tickers, desc="Scanning Geometry"):
        ...
        for i in range(250, len(df)):
            ...
            # בדיקת עוצמה יחסית (RS_65) - מניות שמובילות את השוק בלבד
...
def accept_trades(candidates, data_cache, cfg):
    ...
    for cand in candidates.to_dict("records"):
        ...
        # >>> כאן האכיפה הפיזית של 10 פוזיציות פתוחות בלבד <<<
        if len(active) >= cfg.max_positions: 
            continue
        
        # חישוב שווי התיק הכולל (מזומן + פוזיציות פתוחות)
        current_mkt_val = 0
        for p in active:
...
def print_final_report(overall: dict, yearly_df: pd.DataFrame):
    if not overall or overall.get('Trades', 0) == 0:
        ...
        return
    print("\n" + "=" * 80)
    print("ASCENDING TRIANGLE BACKTEST REPORT (v11)")
    print("=" * 80)
