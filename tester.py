...
class BacktestConfig:
    ...
    output_prefix: str = "canslim_v10_triangle"
...
def get_data(ticker: str, start_fetch: str, end_fetch: str, cfg: BacktestConfig, retries: int = 3) -> pd.DataFrame:
    ...
    for _ in range(retries):
        try:
            # החזרנו ל-yf.Ticker().history כפי שהיה במקור (פותר את שגיאת Pandas)
            ...
            if not df.empty:
                df = df[~df.index.duplicated(keep='first')] # הסרת כפילויות
...
# ==========================================
# 4. Pattern detection (Ascending Triangle / VCP)
# ==========================================
...
def cup_roundness_ok(recent: pd.DataFrame, left_pos: int, bottom_pos: int, right_pos: int) -> bool:
    # נשאר לפונקציות עזר אם נצטרך, אבל משולש מסתמך על שפלים עולים
    return True
