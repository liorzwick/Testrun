...
class BacktestConfig:
    ...
    max_positions: int = 10            # <<< מקסימום 10 פוזיציות במקביל לפי בקשתך
    ...
    # ניהול טרייד וסטופ הדוק (Let Winners Run)
    min_risk_pct: float = 0.01         
    max_risk_pct: float = 0.045        
    max_hold_bars: int = 250           # <<< מאפשרים למניה לרוץ עד שנה!
    ...
    min_rs_65: float = 0.02            # <<< המניה חייבת להכות את המדד ב-2% לפחות!
    ...
    output_prefix: str = "canslim_v11_triangle"
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- תיקון קריטי: שטיח עמודות כפולות מקאש ישן ---
    ...
    # --- תיקון קריטי 2: הסרת כפילויות בימים שגורמות לקריסות ---
...
def get_data(ticker: str, start_fetch: str, end_fetch: str, cfg: BacktestConfig, retries: int = 3) -> pd.DataFrame:
    ...
    # --- תיקון קריטי 3: הוספת "v11" לשם הקובץ כדי לדלג על הקאש הישן ---
    cache_file = cache_dir / f"{ticker}_{start_fetch}_{end_fetch}_{price_tag}_v11.pkl"
...
def get_ascending_triangle_signal(pattern_data: pd.DataFrame, cfg: BacktestConfig):
    ...
    # חוק השפלים העולים מול העמק הראשון
    ...
    for year in tqdm(range(cfg.start_year, cfg.end_year + 1), desc="Years"):
        ...
        for ticker in tickers:
            ...
            for current_date in test_days:
                ...
                # סטופּ הדוק בסיס המשולש (הכיווץ האחרון)
...
def run_canslim_backtest_v7(tickers: list[str], cfg: BacktestConfig):
    ...
    print(f"\nAscending Triangle VCP Backtest: {cfg.start_year}-{cfg.end_year}")
...
def print_report(overall: dict, yearly_df: pd.DataFrame):
    print("\n" + "=" * 80)
    print("ASCENDING TRIANGLE VCP - Backtest Report")
    print("=" * 80)
