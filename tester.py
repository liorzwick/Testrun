...
def simulate_trade(df: pd.DataFrame, entry_date: pd.Timestamp, entry_price: float, initial_stop: float, initial_atr: float, pivot: float, cfg: BacktestConfig):
    ...
    for i, row in enumerate(future.itertuples()):
        ...
        if (i + 1) == cfg.early_exit_bars:
            ...
            if progress < cfg.early_exit_min_progress or day_close < pivot:
                final_exit_date = dt
        ...
        new_stop = stop_today
        if profit_high >= 0.05: new_stop = max(new_stop, entry_price)
        if profit_high >= 0.10: new_stop = max(new_stop, highest_seen - 2.5 * atr_live)
        if profit_high >= 0.15: new_stop = max(new_stop, highest_seen - 2.0 * atr_live)
        if profit_high >= 0.20: new_stop = max(new_stop, highest_seen - 1.5 * atr_live)
...
def run_canslim_backtest_v7(tickers: list[str], cfg: BacktestConfig):
    ...
    print(f"\nCANSLIM v7 Backtest: {cfg.start_year}-{cfg.end_year}")
...
def print_report(overall: dict, yearly_df: pd.DataFrame):
    print("\n" + "=" * 80)
    print("CANSLIM v7 - Backtest Report")
    print("=" * 80)
...
if __name__ == "__main__":
    cfg = BacktestConfig(
        ...
        risk_per_trade=0.005,      # קצת יותר סיכון לעסקה (חצי אחוז)
        max_alloc_pct=0.10,        # מותר להקצות עד 10% הון לכל עסקה
        max_positions=8,           # הגדלנו ל-8 פוזיציות מקבילות במקום 6
        max_portfolio_heat=0.04,   # נאפשר לתיק לקחת עד 4% סיכון כולל
        cooldown_days=20,          # הורדנו את ימי הצינון ל-20
        slippage_bps=12,
        commission_bps=2,
        
        # --- שחרור מסנני התבנית למניות גדולות ---
        breakout_volume_ratio=1.1, # פריצה עם 10% מחזור מעל הממוצע (במקום 50%)
        min_prior_uptrend=0.08,    # מספיק שהמניה עלתה 8% לפני יצירת הספל (במקום 20%)
        min_cup_depth=0.08,        # נאפשר ספלים רדודים יותר של 8%
        max_cup_depth=0.45,        # נאפשר תנודתיות של עד 45% עומק
        max_handle_depth=0.15,     # ידית יכולה לרדת עד 15%
        min_handle_days=3,         # ידית יכולה להיות קצרה של 3 ימים
        max_handle_days=20,
        min_cup_days=25,           # ספל יכול להיות מהיר יותר (25 ימים)
        max_cup_days=180,
        max_pivot_extension=0.05,  # נאפשר למניה לברוח עד 5% מהפיווט ביום הפריצה
        max_entry_extension=0.04,  
        max_gap_above_pivot=0.03,  
        
        # --- ביטול דרישות קיצוניות ---
        min_breakout_close_strength=0.30, # מותר למניה לסגור גם בחלק התחתון ביום הפריצה
        min_rs_65=0.00,                   # לא דורשים שהמניה תכה את המדד בסריקה הראשונית
        max_dist_from_52w_high=0.15,      # נאפשר קנייה עד 15% משיא שנתי
        early_exit_bars=10,
        early_exit_min_progress=0.01,
        min_tight_closes_in_handle=0,     # לא דורשים ימי "סגירה הדוקה" מוחלטת
        
        # --- הגדרות טכניות מותאמות לקולאב ולבקטסט ---
        use_point_in_time_universe=False, # חייב להיות False כשאין קובץ מקומי
        raw_price_mode=False,             # חובה False כדי שספליטים לא יעוותו את הנתונים
        allow_same_day_cash_reuse=False,
        universe_file=None,               # הוגדר כ-None כדי למנוע FileNotFoundError
        output_prefix="canslim_v7_relaxed",
    )

    # משיכת רשימת כל החברות ב-S&P 500 או מקובץ
